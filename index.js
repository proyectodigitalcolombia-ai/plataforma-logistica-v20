...
app.get('/finish/:id', async (req, res) => {
  const ahora = new Date().toLocaleString('es-CO', { timeZone: 'America/Bogota' });
  await C.update({ f_fin: ahora, obs_e: 'FINALIZADO SIN NOVEDAD' }, { where: { id: req.params.id } });
  res.redirect('/');
});
