  ...
  try {
    ...
    res.send(`<html><head><meta charset="UTF-8"><title>LOGISV20</title>${css}</head><body onclick="activarAudio()">
      ...
      <div style="display:flex;gap:10px;margin-bottom:15px;align-items:center">
        <input type="checkbox" id="checkAll" onchange="selectAll(this)"> <label style="color:white">SELECCIONAR TODO</label>
        ...
        <button id="btnDelMult" class="btn-del-mult" onclick="eliminarSeleccionados()">🗑️ ELIMINAR SELECCIONADOS</button>
      </div>
      ...
      function toggleDelBtn(){ const anyChecked = document.querySelectorAll('.row-check:checked').length > 0; document.getElementById('btnDelMult').style.display = anyChecked ? 'block' : 'none'; }
      function eliminarSeleccionados(){ if(!confirm('¿Eliminar despachos seleccionados?')) return; const ids = Array.from(document.querySelectorAll('.row-check:checked')).map(cb => cb.value); fetch('/delete-multiple',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids})}).then(r=>location.reload()); }
