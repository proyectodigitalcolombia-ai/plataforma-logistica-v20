...
// 3. Opciones de Listas (Incluyendo la nueva lista masiva de subclientes)
const opts = {
...
app.get('/', async (req, res) => {
  ...
  res.send(`<html><head><meta charset="UTF-8"><title>LOGISV20</title>${css}</head><body>
    ...
    <form action="/add" method="POST" class="form">
      ...
      <div class="fg"><label>Estado / Obs</label><select name="obs_e">${opts.estados.map(o => `<option value="${o}" ${o==='PENDIENTE'?'selected':''}>${o}</option>`).join('')}</select></div>
