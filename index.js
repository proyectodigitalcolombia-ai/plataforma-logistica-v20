...
const css = `<style>
  ...
  .btn-fin{background:#10b981;color:white;padding:4px 8px;border-radius:4px;text-decoration:none;font-weight:bold;font-size:9px}
</style>`;
...
app.get('/', async (req, res) => {
  try {
    ...
    const rows = d.map(c => {
      ...
      return `<tr><td>${c.id}</td><td>${c.oficina}</td><td>${c.pto}</td><td>${c.do_bl}</td><td>${c.cli}</td><td>${c.subc}</td><td>${c.cont}</td><td>${c.prod}</td><td>${c.orig}</td><td>${c.dest}</td><td>${c.placa||''}</td><td>${c.obs_e}</td><td>${c.est_real}</td><td>${acc}</td><td>${c.f_fin||'--'}</td><td><a href="/d/${c.id}" style="color:#f87171">X</a></td></tr>`;
    }).join('');

    res.send(`<html><head><meta charset="UTF-8"><title>V20 Report Fix</title>${css}</head><body>
      ...
      <button class="btn-xl" onclick="exportExcel()">📥 DESCARGAR EXCEL ORDENADO</button>
      <div class="sc">
        <table id="miTabla">
      ...
      <script>
        function exportExcel(){
          // "sep=;" le dice a Excel que el separador es punto y coma
          let csv = "sep=;\\n"; 
          ...
          for (const row of rows) {
            const cols = Array.from(row.querySelectorAll("td, th")).map(c => {
              ...
              let texto = c.innerText.replace(/\\n/g, ' ').replace(/;/g, ',').trim();
              return '"' + texto + '"';
            });
            // Filtramos la columna de "Acciones" y "Eliminar" para que no salgan en el Excel
            csv += cols.slice(0, 13).join(";") + ";" + cols[14] + "\\n";
          }
          ...
          a.href = url;
          a.download = "Reporte_Logistica_V20.csv";
          a.click();
        }
      </script>
    </body></html>`);
