...
app.get('/', async (req, res) => {
  try {
    ...
    <div id="alert-status">🔊 Audio listo (Haz clic en la pantalla si no suena)</div>
    <h2 style="color:#3b82f6">SISTEMA DE CONTROL LOGÍSTICO V20</h2>
    <div style="display:flex;gap:10px;margin-bottom:15px;align-items:center"><input type="text" id="busq" onkeyup="buscar()" placeholder="🔍 Filtrar por Placa, Contenedor o Cliente..."><button class="btn-xls" onclick="exportExcel()">📥 EXPORTAR A EXCEL</button></div>
    <form action="/add" method="POST" class="form"><datalist id="list_ciud">${opts.ciudades.map(c=>`<option value="${c}">`).join('')}</datalist>
    ...
    <button class="btn">💾 REGISTRAR NUEVA CARGA</button></form>
    <div class="sc fs" id="st"><div class="fc"></div></div>
    <div class="sc" id="sm"><table id="tabla"><thead><tr><th>ID</th><th>FECHA REGISTRO</th><th>OFICINA</th><th>EMPRESA</th><th>COMERCIAL</th><th>PUERTO</th><th>REFLEJA</th><th>FECHA DOC</th><th>HORA DOC</th><th>DO/BL/OC</th><th>CLIENTE</th><th>SUBCLIENTE</th><th>MODALIDAD</th><th>LCL/FCL</th><th>CONTENEDOR</th><th>PESO KG</th><th>UNIDADES</th><th>PRODUCTO</th><th>ESQUEMA</th><th>VENCE</th><th>ORIGEN</th><th>DESTINO</th><th>VEHICULO</th><th>PEDIDO</th><th>F. CARGUE</th><th>H. CARGUE</th><th>F. DESCARGUE</th><th>H. DESCARGUE</th><th>PLACA</th><th>FLETE PAGAR</th><th>FLETE FACT</th><th>ESTADO</th><th>ULTIMA ACTUALIZACION</th><th>ESTADO REAL</th><th style="min-width:300px">OBSERVACIONES</th><th style="min-width:300px">CONDICIONES</th><th>HORARIO</th><th>MUC</th><th>DESPACHADOR</th><th>FECHA FIN</th><th>HORA FIN</th><th>ACCIONES</th></tr></thead><tbody>${rows}</tbody></table></div>
    <script>
    ...
    function buscar(){let f=document.getElementById("busq").value.toUpperCase(),filas=document.getElementById("tabla").getElementsByTagName("tr");for(let i=1;i<filas.length;i++){let tF=filas[i].innerText.toUpperCase();filas[i].style.display=tF.includes(f)?"":"none";}}
    function exportExcel(){let csv="sep=;\\n";document.querySelectorAll("#tabla tr").forEach(row=>{if(row.style.display!=="none"){let cols=Array.from(row.querySelectorAll("td, th")).map(c=>{let inp=c.querySelector('input,select');return '"'+(inp?inp.value:c.innerText).trim()+'"';});csv+=cols.join(";")+"\\n";}});const b=new Blob(["\\ufeff"+csv],{type:"text/csv;charset=utf-8;"}),u=URL.createObjectURL(b),a=document.createElement("a");a.href=u;a.download="Reporte.csv";a.click();}
    
    let audioContext; 
    function activarAudio(){ 
        if(!audioContext) audioContext = new (window.AudioContext || window.webkitAudioContext)();
        if(audioContext.state === 'suspended') audioContext.resume();
        document.getElementById('alert-status').innerHTML = "🟢 Alarma Monitoreando";
        playAlert(); 
    }
    ...
    function playAlert(){ 
        ...
        if(reds.length > 0){ 
            if(!audioContext || audioContext.state === 'suspended') {
                document.getElementById('alert-status').innerHTML = "⚠️ HAZ CLIC AQUÍ PARA ACTIVAR SONIDO";
                return;
            }
            let osc=audioContext.createOscillator(), gain=audioContext.createGain(); 
            osc.type='square'; osc.frequency.setValueAtTime(600, audioContext.currentTime); 
            gain.gain.setValueAtTime(0.15, audioContext.currentTime); 
            gain.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime+0.5); 
            osc.connect(gain); gain.connect(audioContext.destination); 
            osc.start(); osc.stop(audioContext.currentTime+0.5); 
            setTimeout(playAlert, 2500); 
        }
    } 
    window.onload = () => { setTimeout(playAlert, 1500); };
    </script></body></html>`);
