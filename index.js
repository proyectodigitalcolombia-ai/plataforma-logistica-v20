...
app.get('/', async (req, res) => {
  try {
    ...
    for (let c of d) {
      ...
      // BLOQUEO DE FIN: Solo si tiene placa
      let accionFin = c.f_fin 
        ...
        : (tienePlaca 
            ? `<a href="/finish/${c.id}" style="background:#10b981;color:white;padding:3px 6px;border-radius:4px;text-decoration:none;font-size:9px" onclick="return confirm('¿Finalizar viaje?')">FIN</a>` 
            : `<span style="color:#64748b;font-size:8px italic">SIN PLACA</span>`);
      ...
      rows += `<tr class="fila-datos">
        ...
        <td class="col-placa">
          <form action="/u/${c.id}" method="POST" style="margin:0;display:flex;gap:4px;justify-content:center;align-items:center" onsubmit="return valPlaca(this)">
            <input name="placa" class="in-placa" value="${c.placa||''}" ${isLocked} placeholder="PLACA" required oninput="this.value=this.value.toUpperCase()">
        ...
        <td class="col-acc">
          <div class="acc-cell">
            ${tienePlaca ? '<span title="No se puede borrar con placa">🔒</span>' : `<a href="/d/${c.id}" style="color:#f87171;text-decoration:none;font-size:10px" onclick="return confirm('¿Borrar registro?')">🗑️</a>`}
    ...
    res.send(`<html><head><meta charset="UTF-8"><title>LOGISV20</title>${css}</head><body>
      ...
      <form action="/add" method="POST" class="form" style="padding:10px; gap:8px;">
        ...
        <button class="btn">💾 REGISTRAR PRE-ALERTA</button>
      </form>

      <div class="sc" id="sm">
        <table id="tabla">
          <thead><tr><th class="col-num">#</th><th class="col-id">ID</th><th class="col-reg">REGISTRO</th><th>OFICINA</th><th>EMPRESA</th><th>COMERCIAL</th><th>PUERTO</th><th>REFLEJA</th><th>F.DOC</th><th>H.DOC</th><th>DO/BL</th><th>CLIENTE</th><th>SUBCLIENTE</th><th>MODALIDAD</th><th>LCL/FCL</th><th>CONTENEDOR</th><th>PESO</th><th>UNID</th><th>PRODUCTO</th><th>ESQUEMA</th><th>VENCE</th><th>ORIGEN</th><th>DESTINO</th><th>VEHICULO</th><th>PEDIDO</th><th>F.C</th><th>H.C</th><th>F.D</th><th>H.D</th><th class="col-placa">PLACA</th><th>PAGAR</th><th>FACTURA</th><th class="col-est">ESTADO</th><th>ACTUALIZACIÓN</th><th>REAL</th><th>OBSERVACIONES</th><th>CONDICIONES</th><th>HORA</th><th>MUC</th><th>DESPACHADOR</th><th>FIN</th><th class="col-hfin">H.FIN</th><th class="col-acc">ACCIONES</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>

      <script>
      function valPlaca(form){
        const p = form.placa.value.trim();
        if(p.length < 5){ alert("Debe ingresar una placa válida para dar salida."); return false; }
        return true;
      }
      ...
      function buscar(){
        ...
        filas.forEach(fila => {
          ...
          fila.style.display = texto.includes(f) ? "" : "none";
        });
      }
...
app.post('/u/:id', async (req, res) => { 
  if(!req.body.placa) return res.redirect('/');
...
app.get('/reversar/:id', async (req, res) => { await C.update({ f_fin: null, est_real: 'DESPACHADO', obs_e: 'DESPACHADO', f_act: getNow() }, { where: { id: req.params.id } }); res.redirect('/'); });
