...
// Definición del Modelo con todas las columnas de logística
...
const opts = {
  ...
  estados: ['ASIGNADO', 'CANCELADO', 'DESPACHADO', 'EN PROGRAMACIÓN', 'PENDIENTE', 'FINALIZADO']
};
...
const css = `<style>
  ...
  table{border-collapse:collapse;min-width:4500px;font-size:10px}
  th{background:#1e40af;padding:12px;text-align:left;position:sticky;top:0;white-space:nowrap}
  td{padding:8px;border:1px solid #334155;white-space:nowrap}
  .form{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin-bottom:20px;background:#1e293b;padding:20px;border-radius:8px;border:1px solid #2563eb}
  label{font-size:9px;color:#94a3b8;text-transform:uppercase;font-weight:700;display:flex;flex-direction:column;gap:4px}
...
app.get('/', async (req, res) => {
  ...
  const rows = d.map(c => `<tr>
    ...
    <td>${c.oficina||''}</td><td>${c.emp_gen||''}</td><td>${c.comercial||''}</td>
    <td>${c.pto||''}</td><td>${c.refleja||''}</td><td>${c.f_doc||''}</td>
    <td>${c.h_doc||''}</td><td>${c.do_bl||''}</td><td>${c.cli||''}</td>
    <td>${c.subc||''}</td><td>${c.mod||''}</td><td>${c.lcl||''}</td>
    <td>${c.cont||''}</td><td>${c.peso||''}</td><td>${c.unid||''}</td>
    <td>${c.prod||''}</td><td>${c.esq||''}</td><td>${c.vence||''}</td>
    <td>${c.orig||''}</td><td>${c.dest||''}</td><td>${c.t_v||''}</td>
    <td>${c.ped||''}</td><td>${c.f_c||''}</td><td>${c.h_c||''}</td>
    <td>${c.f_d||''}</td><td>${c.h_d||''}</td>
    <td>
      <form action="/u/${c.id}" method="POST" style="margin:0;display:flex;gap:3px">
        ...
        <button style="background:#10b981;color:white;border:none;padding:4px;border-radius:3px;cursor:pointer">OK</button>
      </form>
    </td>
    <td>${c.f_p||''}</td><td>${c.f_f||''}</td>
    <td><span style="background:#475569;padding:4px;border-radius:4px">${c.obs_e||'PENDIENTE'}</span></td>
    <td>${c.f_act||''}</td><td>${c.obs||''}</td><td>${c.cond||''}</td>
    <td>${c.h_t||''}</td><td>${c.muc||''}</td><td>${c.desp||''}</td><td>${c.f_fin||''}</td>
    <td><a href="/d/${c.id}" style="color:#f87171;text-decoration:none" onclick="return confirm('¿Borrar?')">✖</a></td>
  </tr>`).join('');
  ...
  res.send(`<html><head><meta charset="UTF-8"><title>LOGISV20</title>${css}</head><body>
    ...
    <form action="/add" method="POST" class="form">
      ...
      <label>Subcliente<input name="subc" placeholder="Escriba subcliente..."></label>
